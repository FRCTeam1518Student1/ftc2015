...
public class TeleOP extends OpMode {
	...
	// return position of the right flipper servo. note: 0 = 0 degree position and 1 = 180 degree position
	double ServoRFStartPosition = 1;	
	// amount to change the right flipper servo position.
	double ServoRFDelta = 0.25;
	
	// position of the left flipper servo.
	double ServoLFPosition;
	// return position of the left flipper servo. note: 0 = 0 degree position and 1 = 180 degree position
	double ServoLFStartPosition = 0;	
	// amount to change the right flipper servo position.
	double ServoLFDelta = 0.25;
	...
	public void init() {
		 ...
		 *   "motor_5" is on the center right side of the bot.
		 *   "motor_6" is on the center left side of the bot.		 
		 *
		 *
		 *
		 * Motor Layout
		...
		ServoRFPosition = ServoLFStartPosition;  // note: start position = return position
		
		// assign the starting position of the Left Flipper Motor (ServoLF)
		ServoLFPosition = ServoLFStartPosition;  // note: start position = return position
	...
	public void loop() {
		...
		*/
		...
		// assign the current time to the Internal Sweeper Servo Start Time Variable
		long Now = System.currentTimeMillis();
		// Tell the program how long to rotate the Internal Sweeper Servo (1000 = 1 sec.)
		long AllDone = Now + 1000;
		
		if (System.currentTimeMillis() > AllDone) {
		// if the time has passed the time the internal sweeper servo is supposed to run
		// stop the internal sweeper servo
		   CServoIntSPosition = 0.5;
		}
		...
		// Tell the flipper servos to return to the start positions.		
		if (gamepad2.b) {
			// if the B button is pushed on gamepad2, set the position of
			// the right and left flipper servos to the start position.
			ServoRFPosition = ServoRFStartPosition;
			ServoLFPosition = ServoLFStartPosition;			
		}		
			
		// clip the position values so that they never exceed the allowed ranges.
		ServoRFPosition = Range.clip(ServoRFPosition, 0.0, 1.0);  // 0 = 0 degree position and 1 = 180 degree position
		ServoLFPosition = Range.clip(ServoLFPosition, 0.0, 1.0);  // 0 = 0 degree position and 1 = 180 degree position
		
		// write the position values to the flipper servos
		ServoRF.setPosition(ServoRFPosition);  // note: start position = return position
		ServoLF.setPosition(ServoLFPosition);  // note: start position = return position
